...
成员推断攻击（Membership Inference Attack, MIA）：给定已训练模型与一条样本，判断该样本是否出现在训练集中。LLM 场景下用于审计某段文本是否参与预训练/微调、RAG 文档是否被记住等。
...
| 类型 | 攻击者能力 | 典型方法 |
|------|------------|----------|
| 黑盒 | 仅 API 查询（logits/loss/标签） | Shokri Shadow、Yeom Loss、LiRA |
| 灰盒 | 黑盒 + 同架构参考模型 | Ratio（Loss Calibration） |
| 白盒 | 梯度、参数、中间激活 | 联邦梯度 MIA（[Melis et al., 2019](https://arxiv.org/abs/1805.04049)） |
...
Yeom 2018 将 MIA 与泛化误差建立理论联系，是后续 LLM Loss 攻击的基础。
...
Ratio / Loss Calibration（Watson 2022，ICLR）：用参考模型剥离样本难度，\(\text{score} = \ell^{\text{R}} / \ell^{\text{T}}\)，是 LLM 灰盒 MIA 的强基线。
...
| ZLIB / Lowercase | 损失经压缩长度或大小写变换归一化 | 缓解长度/表面形式偏差（Carlini 2021） |
...
| 选择性复述 (SOFT) | 高泄漏样本 paraphrase | SAMA 实验有效 |
...
| [Li & Zhang, 2025](https://doi.org/10.1051/sands/2024017) | Security and Safety，MIA 现状与未来方向 |
...
| [Jia et al., 2019](https://arxiv.org/abs/1806.12136) | CCS | MemGuard，置信度加噪防御 |
