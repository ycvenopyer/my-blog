...
**MCP让Claude能碰到外部系统，Skills告诉Claude碰到之后怎么用**
...
Skills是**使用手册**。当有重复性的工作流程时使用。

两者互补，在一个复杂的场景下需要同时使用。

Skills比MCP更简洁**（只需markdown+YAML元数据和一些脚本）**，tokens消耗更少，跨平台兼容，且门槛低生态好。

Skills的设计符合LLM的本质：**用文本描述能力，让模型理解并执行。**

MCP像是传统软件工程的思路：定义接口、实现服务、处理通信。
...
**如何创建Skill**：Skill封装了你的工作流程、你的经验沉淀、你的SOP，将这些东西写成SKILL.md，让AI来做即可。
