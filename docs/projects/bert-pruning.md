...
- **LayerDrop** — Fan 等，[Reducing Transformer Depth on Demand with Structured Dropout](https://arxiv.org/abs/1909.11556)。训练时以一定概率随机丢弃整层，使子网络在多种深度上都能工作；推理时可按需使用更浅的子网络，从而在深度维度上获得压缩与加速潜力。  
...
![bert-pruning](image/bert-pruning/bert-pruning.png)
...
![layer-pruning](image/bert-pruning/layer-pruning.png)
...
![head-pruning](image/bert-pruning/head-pruning.png)

### 3.2 实验现象

![head-pruning-result](image/bert-pruning/head-pruning-result.png)
...
![CoFi](image/bert-pruning/CoFi.png)
...
![CoFi-result](image/bert-pruning/CoFi-result.png)

在尝试向 CoFi 论文设定靠拢时，我们采用约0.2 的剪枝率、稀疏度约 0.2 等配置，观察到：
