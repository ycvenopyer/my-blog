...
STE 的核心问题：同一 Voronoi 区域内所有 \(e\) 获得相同梯度，与 \(e\) 距 \(q\) 的远近、相对夹角无关。位置信息丢失，易导致码本利用率低、码本崩塌（大量码字范数趋零）以及量化误差 \(\|e-q\|_2^2\) 偏大。完整符号、链式法则与替代方案见 §2。
...
\[
\frac{\partial L}{\partial x} = \underbrace{\frac{\partial L}{\partial q}}_{\text{解码器}} \underbrace{\frac{\partial q}{\partial e}}_{\text{量化层}} \underbrace{\frac{\partial e}{\partial x}}_{\text{编码器}}
\]
...
论文还讨论了两种看似更“精确”的替代：Hessian 近似与双前向精确梯度。在 \(q\) 处 Taylor 展开可得

\[
\nabla_e L \approx \nabla_q L + (\nabla_q^2 L)(e - q)
\]

或在 \(q=e\) 的辅助前向中计算精确 \(\nabla_e L\)。二者往往表现更差：对 \(e\) 的精确梯度等价于训练普通自编码器，与 VQ-VAE 的离散瓶颈归纳偏置不匹配；当 \(e \approx q\) 时应有 \(\nabla_e L \approx \nabla_q L\)，Hessian/精确梯度在负曲率或鞍点处可能违背该假设。
...
### 2.5 Householder 高效实现

显式构造 \(d \times d\) 的 \(R\) 开销大。用 Householder 反射可写成仅含向量内积的形式。令 \(\lambda=\|q\|/\|e\|\)，\(r=(\hat{e}+\hat{q})/\|\hat{e}+\hat{q}\|\)：

\[
\tilde{q} = \lambda R e = \lambda\bigl(I - 2rr^\top + 2\hat{q}\hat{e}^\top\bigr) e = \lambda\bigl[e - 2r(r^\top e) + 2\hat{q}(\hat{e}^\top e)\bigr]
\]

等价于两次反射的复合：\(R = (I-2rr^\top)(I-2\hat{q}\hat{q}^\top)\) 在 \(\hat{e},\hat{q}\) 平面内将 \(\hat{e}\) 旋转至 \(\hat{q}\)（附录 A.5）。该写法避免大矩阵外积，GPU 显存占用极小；墙钟时间与 STE 无显著差异。
...
- Hessian 近似或精确梯度替代 STE 表现更差（编码器趋近自编码器，优化目标不匹配）。

视频 VQ-VAE：STE 训练出现码本崩塌（BAIR 批内约 4/1024 码字被用）；rotation trick 稳定（约 441/1024），重建 r-FVD 更优。
...
- 与 SimVQ（码本线性变换）等互补技巧组合；  
  
- 分析 rotation trick 对 Stage-2 生成质量（而非仅重建）的下游影响——论文未覆盖此点。
