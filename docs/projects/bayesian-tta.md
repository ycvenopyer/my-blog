...
![BTTA 整体框架](image/bayesian-tta/btta-framework.png)
...
在线场景中，早期预测不可靠会导致似然估计有偏。本方法为每类维护容量为 $C$ 的知识库 $\mathcal{K}_k$，以 CLIP 预测负熵作为置信度，仅保留高置信样本。
...
$$
\phi = \alpha \cdot H_{\text{norm}} + (1-\alpha) \cdot (1 - p_{\max})
$$

融合系数 $\lambda(\phi)$ 在 $[\lambda_{\min}, \lambda_{\max}]$ 间线性变化：高置信样本更依赖知识库，含糊样本更回退到 CLIP 先验。

### 4.4 直推式扩展

直推模式下可一次性访问整个测试集，将在线目标扩展为全局目标，标签估计器形式不变；类均值改为单遍估计，$\lambda$ 取固定值（无需动态更新）。实验表明直推模式准确率通常高于在线模式，但在线模式更贴近真实流式部署。
...
- 超参数（ViT-B/16）：在线模式 $\lambda \in [0.6, 0.9]$，$\gamma \in [0.9, 1.0]$，$\alpha=0.5$，知识库大小 $C=16$；直推模式 $\lambda=0.9$，$C=6$
...
知识库过小则统计信息不足、性能波动；适中容量后趋于稳定。直推模式因可访问全测试集，对 $C$ 更鲁棒。

![直推模式下 λ 的影响](image/bayesian-tta/alpha-beta.png)

直推模式准确率随 $\lambda$ 增大稳步提升，说明方法更受益于可靠历史样本而非仅依赖静态 CLIP 先验；取 $\lambda=0.9$ 可平衡先验与自适应性。
